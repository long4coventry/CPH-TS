...
def cph1 (data_x, cph_parameters,data_image):
  seed = 25
  random.seed(seed)
  np.random.seed(seed)
  ...
  # print(Z_mb,"6666666666666666666666666666666")
  M_mb = data_m
  X_mb = norm_data_x          
  X_mb = M_mb * X_mb + (1-M_mb) * Z_mb 
  image_mb = data_image
  ...
  D_prob = sess.run([D_prob*M-D_prob1*M], feed_dict = {M: M_mb, X_pre: image_mb, H: H_mb})

  original_imputed_data = imputed_data*data_m
  # print(original_imputed_data,'#########################',original_imputed_data.shape)
  ...
  original_imputed_data = rounding(original_imputed_data,data_x) 
